...
void NET_DEVICE_IO_Init(void)
{
	...
	gpioInitStruct.GPIO_Pin = GPIO_Pin_5;					//GPIOB5-复位
...
unsigned char *NET_DEVICE_GetIPD(unsigned char *dataPtr)
{
	...
	if(dataPtr_tp != NULL)
	{
		...
		else
		{
			...
			while(*dataPtr_tp < '0' || *dataPtr_tp > '9')			//找到第一个为数字的字符
			{
				if(*dataPtr_tp == '\0')	return NULL;
				
				dataPtr_tp++;
			}
			while(*dataPtr_tp >= '0' && *dataPtr_tp <= '9')			//复制
				bytes[i++] = *dataPtr_tp++;
			
			netDeviceInfo.data_bytes = atoi(bytes);					//转换
