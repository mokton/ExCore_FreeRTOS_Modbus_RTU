	...
	if(dataPtr != NULL)												//数据有效
	{
		...
		if(ipdPtr != NULL)
		{
			...
			if(netDeviceInfo.netWork)
				OneNet_RevPro(ipdPtr, netDeviceInfo.data_bytes);	//集中处理
			else
...
unsigned char OneNet_AddDataSendList(unsigned short *buf ,unsigned short dataLen)
{
	...
	if(oneNetInfo.head == NULL)										//如果head为NULL
		oneNetInfo.head = current;									//head指向当前分配的内存区
	else															//如果head不为NULL
		oneNetInfo.end->next = current;								//则end指向当前分配的内存区
	
	memcpy(current->buf, buf, dataLen << 1);						//复制数据
	current->dataLen = dataLen;
	current->next = NULL;											//下一段为NULL
	
	oneNetInfo.end = current;										//end指向当前分配的内存区
...
_Bool OneNet_DeleteDataSendList(void)
{
	...
	struct MODBUS_LIST *next = oneNetInfo.head->next;		//保存链表头的下一段数据地址
	
	oneNetInfo.head->dataLen = 0;
	oneNetInfo.head->next = NULL;
	NET_FreeBuffer(oneNetInfo.head->buf);					//释放内存
	NET_FreeBuffer(oneNetInfo.head);						//释放内存
	
	oneNetInfo.head = next;									//链表头指向下一段数据
