...
void IIC_Init(void)
{
	...
	gpioInitStruct.GPIO_Pin = I2C_SCL_Pin | I2C_SDA_Pin;
...
_Bool I2C_WriteByte(unsigned char slaveAddr, unsigned short regAddr, unsigned char *byte)
{
	...
	IIC_SendByte(regAddr >> 8);	//发送寄存器地址 高位
	if(IIC_WaitAck(5000))		//等待应答
		return IIC_Err;
	
	IIC_SendByte(regAddr % 256);//发送寄存器地址 低位
	if(IIC_WaitAck(5000))		//等待应答
		return IIC_Err;
...
************************************************************
*/
_Bool I2C_ReadByte(unsigned char slaveAddr, unsigned short regAddr, unsigned char *val)
{
	...
	IIC_SendByte(regAddr >> 8);	//发送寄存器地址
	if(IIC_WaitAck(5000))		//等待应答
		return IIC_Err;
	
	IIC_SendByte(regAddr % 256);//发送寄存器地址
	if(IIC_WaitAck(5000))		//等待应答
		return IIC_Err;
...
************************************************************
*/
_Bool I2C_WriteBytes(unsigned char slaveAddr, unsigned short regAddr, unsigned char *buf, unsigned char num)
{
...
************************************************************
*/
_Bool I2C_ReadBytes(unsigned char slaveAddr, unsigned short regAddr, unsigned char *buf, unsigned char num)
{
